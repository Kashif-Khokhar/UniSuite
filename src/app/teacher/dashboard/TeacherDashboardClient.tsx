...
import { 
  ...
  ChevronDown, ArrowRight, Bell, GraduationCap, User, Layers
} from "lucide-react";
import { 
...
export default function TeacherDashboardClient({ teacher }: { teacher: any }) {
  ...
  const totalStudents = teacher.sections.reduce((acc: number, sec: any) => acc + sec.enrollments.length, 0);
        ...
        <div className="p-6 md:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                <h1 className="text-2xl font-bold tracking-tight md:text-3xl">{teacher.name}-{teacher.employeeId}</h1>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:gap-4">
              {[
                { label: "SECTIONS", value: teacher.sections.length },
                { label: "STUDENTS", value: totalStudents },
                { label: "WORKLOAD", value: teacher.sections.reduce((acc: number, sec: any) => acc + sec.course.creditHours, 0) + " CR" },
                ...
                <div key={stat.label} className="flex flex-col items-center justify-center rounded-xl bg-white/10 p-4 backdrop-blur-md border border-white/10">
                  <span className="text-2xl font-bold whitespace-nowrap">{stat.value}</span>
          ...
          {/* Bottom Details Row */}
          <div className="mt-8 flex flex-wrap gap-x-8 gap-y-4 border-t border-white/10 pt-6 text-sm">
            <div className="min-w-0 flex-1 sm:flex-none">
              <p className="text-[10px] font-bold uppercase tracking-wider text-white/50">Email</p>
              <p className="mt-1 font-medium break-all sm:break-normal">{teacher.email}</p>
            </div>
            <div className="min-w-0 flex-1 sm:flex-none">
              <p className="text-[10px] font-bold uppercase tracking-wider text-white/50">Mobile</p>
              <p className="mt-1 font-medium">{teacher.mobile || "N/A"}</p>
            </div>
            <div className="min-w-0 flex-1 sm:flex-none">
              <p className="text-[10px] font-bold uppercase tracking-wider text-white/50">Nationality</p>
              <p className="mt-1 font-medium">{teacher.nationality || "N/A"}</p>
            </div>
            <div className="min-w-0 flex-1 sm:flex-none">
            ...
            <div className="min-w-0 flex-1 sm:flex-none">
              ...
              <p className="mt-1 font-medium">{teacher.createdAt?.toLocaleDateString() || "N/A"}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col h-full">
              <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-800">Assigned Sections</h3>
                    <p className="text-[10px] text-slate-500">Current Workload</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-bold text-slate-800">{teacher.sections.length}</p>
                  <p className="text-[10px] font-bold text-slate-500">Sections</p>
                </div>
              </div>
              
              <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                {teacher.sections.length > 0 ? teacher.sections.map((section: any) => (
                  <div key={section.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 bg-slate-50">
                    <div>
                      <p className="font-bold text-slate-800 text-sm">{section.course.name}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs font-semibold text-brand-600 bg-brand-50 px-2 py-0.5 rounded">{section.name}</span>
                        <span className="text-[10px] text-slate-500">{section.enrollments.length} Enrolled</span>
                      </div>
                    </div>
                  </div>
                )) : (
                  <div className="flex h-32 flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50 text-slate-400">
                    ...
                    <p className="text-sm font-semibold text-slate-600">No Assigned Sections</p>
                  </div>
                )}
              </div>
            </div>
