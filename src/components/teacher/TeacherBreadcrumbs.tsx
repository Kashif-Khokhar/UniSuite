...
function resolveTeacherBreadcrumb(pathname: string) {
  ...
  const segments = pathname.replace(/^\/teacher\/dashboard\/?/, "").split("/").filter(Boolean);
...
export default function TeacherBreadcrumbs() {
  ...
  const trail = resolveTeacherBreadcrumb(pathname);
